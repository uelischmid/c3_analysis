...
gbm.plot_us <- function (gbm.object, variable.no = 0, smooth = FALSE, rug = TRUE, 
...
gbm.plot_us_invlogit <- function (gbm.object, variable.no = 0, smooth = FALSE, rug = TRUE, 
                                  n.plots = length(pred.names), common.scale = TRUE, write.title = TRUE, 
                                  y.label = "fitted function", x.label = NULL, show.contrib = TRUE, 
                                  plot.layout = c(3, 4), ...) {
  if (!requireNamespace("gbm")) {
  ...
  for (j in c(1:n.plots)) {
    ...
    responses[[j]] <- boot::inv.logit(response.matrix[, 2]) - boot::inv.logit(mean(response.matrix[, 2])) # backtransformation
    if (j == 1) {
      ymin = min(responses[[j]])
      ymax = max(responses[[j]])
    } else {
