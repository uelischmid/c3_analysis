...
plot_mgmi <- function(m_all, mc_all,
                      st, rv) {
  mc_used <- mc_all %>% 
    filter(simtype == st) %>% 
    filter(resp_var_type == rv)
  ...
  mgm_contribution <- contributions %>% 
    mutate(mgm_var = case_when(var %in% c("Type", "Interval", "Intensity") ~ "mgm",
                               TRUE                                        ~ "other")) %>% 
    group_by(label, mgm_var) %>% 
    summarise(rel.inf.sum = sum(rel.inf),
              .groups     = "drop")
  
  title_str <- str_c(st, " - ", rv)
  
  gg_p <- mgm_contribution %>% 
    filter(mgm_var == "mgm") %>% 
    ggplot(aes(label, rel.inf.sum)) +
    geom_col() +
    coord_flip() +
    labs(title = title_str,
         x     = NULL,
...
plot_mgmi(m_all  = models_full,
          mc_all = model_combinations,
          st     = "LT",
          rv     = "abs")
ggsave(str_c(folder_out, "LT_abs_full_mgm.png"))
...
plot_mgmi(m_all  = models_full,
          ...
          st     = "LT",
          rv     = "diff")
ggsave(str_c(folder_out, "LT_diff_full_mgm.png"))
...
plot_mgmi(m_all  = models_full,
          ...
          st     = "ST",
          rv     = "abs")
ggsave(str_c(folder_out, "ST_abs_full_mgm.png"))
...
plot_mgmi(m_all  = models_full,
          ...
          st     = "ST",
          rv     = "diff")
ggsave(str_c(folder_out, "ST_diff_full_mgm.png"))
