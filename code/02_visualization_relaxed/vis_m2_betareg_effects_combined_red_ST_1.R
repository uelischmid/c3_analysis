...
folder_out <- "results/vis_models_v2_relaxed/betareg/vis_betareg_red_effects_combined_1/"
...
plot_effects_comb <- function(st, rvt, stra, i_sel,
  ...
  effs_all <- bind_rows(effs_quality,
                        effs_mgm_type,
                        effs_mgm_intint1 %>% 
                          mutate(x = factor(x)),
                        effs_mgm_intint2 %>% 
  ...
  # name plots
  p_title_plot <- plot_profilename(str_c(st, rvt, stra, i_sel, sep = " "))
