...
folder_out <- "results/vis_models_v2_relaxed/betareg/vis_betareg_red_effects_combined_3b/"
...
plot_effs <- function(stra, st_init,
  ...
  # plot all
  plots_left <- plot_grid(gg_qsite, gg_type,
                          ncol   = 1,
                          labels = c("a", "b"))
  
  plots_middle <- plot_grid(gg_intint,
                            labels = c("c"))
  
  plots_right <- plot_grid(legend_nathaz, legend_deltapq,
                           ncol = 1)
  
  plots_all <- plot_grid(plots_left, plots_middle, plots_right,
                         ncol       = 3,
                         rel_widths = c(1.5, 1, 0.3))
