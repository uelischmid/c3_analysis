...
plot_mgm_4cs_red <- function(stratum_selected,
                             init_selected,
                             nathaz_selected,
                             quality_selected,
                             response_var,
  ...
  if (response_var == "sha_i_MP_met") {
    title_part <- "MP"
  } else if (response_var == "sha_i_IP_met") {
    title_part <- "IP"
  } else {
    title_part <- "Error"
  }
...
plot_mgm_4cs_red2 <- function(stratum_selected,
                              init_selected,
                              nathaz_selected,
                              quality_selected,
                              response_var,
  ...
  dat_red_best <- dat_red %>% 
    filter(.data[[response_var]] == max(.data[[response_var]]))
...
plot_mgm_4cs_all_red <- function(stratum_sel,
                                 init_sel,
                                 nathaz_sel,
                                 # resp_var_sel,
                                 res) {
  ...
  plots <- vector(mode = "list", length = 4)
  for (i in seq_along(levels(res$quality))) {
    plots[[i]] <- plot_mgm_4cs_red(stratum_selected = stratum_sel,
                                   init_selected    = init_sel,
                                   nathaz_selected  = nathaz_sel,
                                   quality_selected = levels(res$quality)[i],
                                   response_var     = "sha_i_MP_met",
                                   dat              = res)
  }
  
  for (i in seq_along(levels(res$quality))) {
    plots[[i + 2]] <- plot_mgm_4cs_red(stratum_selected = stratum_sel,
                                       init_selected    = init_sel,
                                       nathaz_selected  = nathaz_sel,
                                       quality_selected = levels(res$quality)[i],
                                       response_var     = "sha_i_IP_met",
                                       dat              = res)
  }
...
plot_mgm_4cs_all_red2 <- function(stratum_sel,
                                  init_sel,
                                  nathaz_sel,
                                  # resp_var_sel,
                                  res) {
  ...
  plots <- vector(mode = "list", length = 4)
  for (i in seq_along(levels(res$quality))) {
    plots[[i]] <- plot_mgm_4cs_red2(stratum_selected = stratum_sel,
                                    init_selected    = init_sel,
                                    nathaz_selected  = nathaz_sel,
                                    quality_selected = levels(res$quality)[i],
                                    response_var     = "sha_i_MP_met",
  ...
  for (i in seq_along(levels(res$quality))) {
    plots[[i + 2]] <- plot_mgm_4cs_red2(stratum_selected = stratum_sel,
                                        init_selected    = init_sel,
                                        nathaz_selected  = nathaz_sel,
                                        quality_selected = levels(res$quality)[i],
                                        response_var     = "sha_i_IP_met",
